...
for i in range(len(az[0])):		
	az[5].append(np.cos(phi-beta)*np.cos(az[2][i])*np.cos(az[3][i])+np.sin(phi-beta)*np.sin(az[2][i])) #costheta
	az[6].append(np.cos(phi)*np.cos(az[2][i])*np.cos(az[3][i])+np.sin(phi)*np.sin(az[2][i]))#coszenith
#	if az[6][i]< 0.1:
#		az[6][i]=0.1#np.absolute(az[5][i])
...
#	print (az[3][i],az[7][i],az[10][i],az[8][i])#,az[11][15],az[12][15],az[1][15])
print sum
